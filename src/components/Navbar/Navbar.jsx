...
const Navbar = ({ title }) => {
  ...
  const isQuotesPage = location.pathname.includes('/quotes-load') || location.pathname.includes('/quote-load');
  const navbarBackground = isQuotesPage ? 'bg-[#FBF0F4]' : 'bg-[#EBE6E0]';
  const navbarTextColor = isQuotesPage ? 'text-[#91736B]' : 'text-[#277a59]';
  const navbarBorderColor = isQuotesPage ? 'border-[#91736B]' : 'border-[#277a598a]';

  const menuItems = dropdownOptions[location.pathname] || dropdownOptions['/'];

  return (
    <div
      className={`navbar fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${navbarBackground} ${navbarBorderColor}
        ${isScrolled ? `${navbarBackground}/80 backdrop-blur-md shadow-lg` : 'border-b-2'}`}
    >
      <div className="navbar-start">
        <div className="dropdown" ref={dropdownRef}>
          <button onClick={handleMenuToggle} className={`btn btn-ghost btn-circle ${navbarTextColor}`}>
            {menuOpen ? <FiX className="h-5 w-5" /> : <FiMenu className="h-5 w-5" />}
          </button>
          {menuOpen && (
            <ul
              className={`menu menu-sm dropdown-content bg-white/30 border ${navbarBorderColor} ${navbarTextColor} backdrop-blur-md rounded-box z-[1] mt-3 w-52 p-2 shadow-lg`}
            >
      ...
      <div className="navbar-center relative group">
        <div className={`btn btn-ghost normal-case text-xl ${navbarTextColor} flex items-center`}>
        ...
        <div className="absolute top-12 left-0 w-full bg-white border border-gray-200 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 mt-2">
          <ul className="text-gray-700">
            {menuItems.map((item) => (
              <li key={item.path}>
                <Link to={item.path} className={`block px-4 py-2 hover:bg-gray-100 ${navbarTextColor} font-bold text-xl`}>
                  {item.label}
                </Link>
              </li>
            ))}
      ...
      <div className="navbar-end">
        <a
          ...
          className={`btn btn-ghost ${navbarTextColor} text-[3rem] btn-circle`}
        >
